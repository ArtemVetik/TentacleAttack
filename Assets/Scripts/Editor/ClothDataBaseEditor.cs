...
[CustomEditor(typeof(ClothDataBase))]
public class ClothDataBaseEditor : Editor
{
    ...
    {
        _dataBase = target as ClothDataBase;

        FieldInfo[] allFields = _dataBase.GetType().GetFields(BindingFlags.NonPublic | BindingFlags.Instance);
        _dataBaseListInfo = allFields.ToList().Find((field) => field.FieldType.IsEquivalentTo(typeof(List<ClothData>)));
    ...
    public override void OnInspectorGUI()
    {
        ...
        GUILayout.FlexibleSpace();
        GUILayout.EndHorizontal();
        EditorGUILayout.LabelField("Default Accessory index: 0!");
        GUILayout.Space(30);
